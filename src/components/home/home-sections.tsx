...
const categoryImages = [
  ["Kadın Parfümleri", "Zarafetin unutulmaz ifadesi", "Kadın", "https://images.unsplash.com/photo-1592945403244-b3fbafd7f539?auto=format&fit=crop&w=1200&q=85"],
  ["Erkek Parfümleri", "Güçlü ve karakterli seçimler", "Erkek", "https://images.unsplash.com/photo-1615634260167-c8cdede054de?auto=format&fit=crop&w=1200&q=85"],
  ...
  ["Niş Parfümler", "Nadir ve özgün kompozisyonlar", "", "https://images.unsplash.com/photo-1523293182086-7651a899d37f?auto=format&fit=crop&w=1200&q=85"],
];
const families = [
...
export function CategorySection() {
  return <section className="section-shell"><SectionTitle eyebrow="KENDİNİZE ÖZEL" title="Koleksiyonları Keşfedin" /><div className="grid gap-4 md:grid-cols-2">{categoryImages.map(([title, text, category, image], index) => <Link href={`/urunler${category ? `?category=${category}` : ""}`} key={title} className={`image-card group relative overflow-hidden ${index < 2 ? "aspect-[4/3]" : "aspect-[5/3]"}`}><Image src={image} alt="" fill className="object-cover transition duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 50vw" /><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent transition group-hover:bg-black/30" /><div className="absolute inset-x-0 bottom-0 p-7 text-white md:p-10"><h3 className="font-serif text-3xl md:text-4xl">{title}</h3><p className="mt-2 text-xs text-white/75">{text}</p><span className="mt-5 inline-block border-b border-white pb-1 text-[10px] tracking-[.2em]">KEŞFET</span></div></Link>)}</div></section>;
}
...
export function ScentFamilies() {
  return <section className="section-shell"><SectionTitle eyebrow="KOKUNUN DİLİ" title="Koku Aileleri" /><div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">{families.map(([title, text, image]) => <Link href={`/urunler?family=${title}`} key={title} className="group text-center"><div className="relative mx-auto aspect-square w-full overflow-hidden rounded-full"><Image src={image} alt="" fill className="object-cover transition duration-700 group-hover:scale-110" sizes="17vw" /></div><h3 className="mt-5 font-serif text-xl">{title}</h3><p className="mx-auto mt-1 max-w-40 text-xs leading-5 text-neutral-500">{text}</p></Link>)}</div></section>;
}
...
export function Campaigns() {
  ...
  return <section id="kampanyalar" className="section-shell"><SectionTitle eyebrow="SADECE SİZE ÖZEL" title="Luméa Ayrıcalıkları" /><div className="grid gap-4 md:grid-cols-2">{cards.map(([title, text, image], i) => <Link href="/urunler" key={title} className={`group relative overflow-hidden ${i === 0 ? "min-h-[570px] md:row-span-2" : "min-h-[277px]"}`}><Image src={image} alt="" fill className="object-cover transition duration-700 group-hover:scale-105" sizes="50vw" /><div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" /><div className="absolute bottom-0 p-7 text-white"><p className="text-xs text-white/65">{text}</p><h3 className="mt-2 max-w-md font-serif text-3xl">{title}</h3><span className="mt-4 inline-block text-[10px] tracking-widest underline underline-offset-4">FIRSATI KEŞFET</span></div></Link>)}</div></section>;
}
...
export function TrustAndReviews() {
  ...
  const reviews = [["Elif A.", "Coco Mademoiselle", "Paketleme başlı başına bir deneyimdi. Ürün tamamen orijinal ve teslimat çok hızlıydı."], ["Mert K.", "Sauvage Elixir", "Koku danışmanının önerisi tam isabet oldu. Kalıcılığı ve performansı harika."], ["Deniz S.", "Libre Intense", "Luméa artık parfüm alışverişindeki tek adresim. Her detay çok özenli."]];
  return <><section id="hakkimizda" className="border-y border-black/10 bg-white py-12"><div className="mx-auto grid max-w-[1500px] grid-cols-2 gap-8 px-5 md:grid-cols-5 lg:px-8">{perks.map(([Icon, title, text]) => { const I = Icon as typeof ShieldCheck; return <div key={title as string} className="text-center"><I className="mx-auto text-[#9d7747]" strokeWidth={1.2} /><h3 className="mt-3 font-serif text-lg">{title as string}</h3><p className="mt-1 text-[11px] text-neutral-500">{text as string}</p></div>; })}</div></section><section className="section-shell"><SectionTitle eyebrow="SİZDEN GELENLER" title="Luméa Deneyimleri" /><div className="grid gap-4 md:grid-cols-3">{reviews.map(([name, product, text]) => <article key={name} className="border border-black/10 bg-white p-7 md:p-9"><div className="flex gap-1 text-[#b68c55]">{Array.from({length:5}).map((_,i)=><Star key={i} size={13} fill="currentColor" />)}</div><blockquote className="mt-6 font-serif text-xl leading-8">“{text}”</blockquote><div className="mt-7 border-t border-black/10 pt-5"><div className="flex items-center justify-between"><b className="text-sm">{name}</b><span className="flex items-center gap-1 text-[9px] text-emerald-700"><BadgeCheck size={13} /> DOĞRULANMIŞ</span></div><p className="mt-1 text-xs text-neutral-500">{product}</p></div></article>)}</div></section></>;
}
...
export function Newsletter() {
  ...
  const submit = ({ email }: { email: string }) => { const result = emailSchema.safeParse(email); if (!result.success) return setError("email", { message: result.error.issues[0].message }); toast.success("Luméa dünyasına hoş geldiniz!"); reset(); };
  return <section className="border-t border-black/10 bg-[#eee8dc] py-20"><div className="mx-auto max-w-2xl px-5 text-center"><p className="text-[10px] tracking-[.3em] text-[#8d693e]">LUMÉA PRIVÉ</p><h2 className="mt-4 font-serif text-4xl md:text-6xl">Luméa Dünyasına Katılın</h2><p className="mx-auto mt-4 max-w-xl text-sm leading-6 text-neutral-600">Yeni koleksiyonlardan, kampanyalardan ve özel fırsatlardan ilk siz haberdar olun.</p><form onSubmit={handleSubmit(submit)} className="mx-auto mt-8 flex max-w-xl border-b border-black"><input {...register("email")} aria-label="E-posta adresi" placeholder="E-posta adresiniz" className="min-w-0 flex-1 bg-transparent px-1 py-4 text-sm outline-none" /><button className="px-4 text-[10px] tracking-[.18em]">ABONE OL <ArrowRight className="ml-2 inline" size={14} /></button></form>{errors.email && <p className="mt-2 text-left text-xs text-red-700">{errors.email.message}</p>}</div></section>;
}
